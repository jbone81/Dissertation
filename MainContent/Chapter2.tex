...
Earlier systematic reviews by \citeauthor{Henderson_Salado_2021} examined MBSE value and maturity across industrial contexts~\cite{Henderson_Salado_2021}. That review categorized reported benefits into four evidence types---measured, observed, perceived, and referenced---revealing that approximately two-thirds of claimed MBSE benefits lack empirical measurement and instead rely upon perceived or referenced evidence. This evidence gap carries implications for adoption decisions: organizations evaluating MBSE investment lack the quantified outcomes they require for justification. Among the rare exceptions, \citeauthor{Rogers_Mitchell_2021} documented measured return on investment following MBSE adoption on the Submarine Warfare Federated Tactical Systems program, reporting an eighteen percent improvement in systems engineering efficiency and a nine percent reduction in interface defects~\cite{Rogers_Mitchell_2021}. The scarcity of such quantified evidence across the broader literature contributes to economic barriers that impede adoption decisions, particularly in domains such as enterprise IT where no domain-specific evidence exists. \citeauthor{Wolny_2020} reviewed empirical evidence for model-based methods~\cite{Wolny_2020}. \citeauthor{Chami_Bruel_2018} surveyed MBSE tools and applications~\cite{Chami_Bruel_2018}. These reviews collectively establish that MBSE has demonstrated value across multiple domains while documenting the absence of enterprise IT applications.

\subsection{Empirical Studies of MBSE Implementation}

Research by \citeauthor{gregory2019model} \& \citeauthor{Henderson_Salado_2021} examined model-based engineering practices within defense programs, documenting improved requirements traceability and more effective design reviews but also identifying organizational and technical barriers to adoption~\cite{gregory2019model,Henderson_Salado_2021}. The empirical evidence indicates that MBSE provides value in traditional systems engineering domains. However, the transferability of these benefits to enterprise IT and Information Assurance domains remains unexamined.

Recent empirical research provides deeper understanding of the dynamics governing MBSE adoption. \citeauthor{Call_2024} surveyed approximately 270 systems engineering professionals, applying Everett Rogers' Diffusion of Innovations theory to examine how perceptions of MBSE attributes influence adoption decisions~\cite{Call_2024}. The study examined five innovation attributes---relative advantage, compatibility, complexity, trialability, and observability---that collectively account for forty-nine to eighty-seven percent of variance in adoption rates across innovation research. The findings reveal that perceived compatibility with existing practices and perceived complexity significantly impede adoption. Critically, respondents not involved in MBSE efforts reported dramatically lower access to trial opportunities ($p=0.00$), creating a barrier-reinforcing cycle: professionals who have not experienced MBSE cannot access the trial opportunities that would facilitate informed adoption decisions. The study characterizes MBSE as a ``preventative innovation'' whose advantages derive from preventing problems---inconsistencies, documentation errors, rework---rather than producing visible new benefits. This characteristic depresses both perceived relative advantage and observability, compounding adoption barriers even when objective value exists.

\citeauthor{Henderson_McDermott_Salado_2024} conducted a mixed-methods study combining systematic literature review with eighteen semi-structured practitioner interviews from organizations attempting MBSE adoption, including two organizations whose adoption efforts failed and one at standstill~\cite{Henderson_McDermott_Salado_2024}. The literature review extracted over six hundred individual lessons learned from forty-six published papers, coded into categories spanning communication, model definition, organizational strategy, and technical implementation. The interview findings reveal a striking observation directly relevant to professional awareness: approximately twenty-two percent of interview participants---all systems engineering professionals---could not convey a clear definition of MBSE\@. One participant stated plainly that most practitioners ``think of MBSE as being synonymous with a specific tool'' and ``don't understand how MBSE fits into DE or what it really does other than it is modeling instead of documents.'' For both organizations that failed to adopt MBSE, the driving barrier was lack of management support, with middle management resistance proving particularly insidious because executives endorsed MBSE while middle managers could disregard the endorsement without operational consequences.

The adoption barriers identified in MBSE research warrant attention because similar barriers likely impede adoption in enterprise IT contexts. Cultural resistance to new methodologies, tool learning curves, initial productivity decreases during transition, and integration challenges with existing processes all affected MBSE adoption in aerospace and defense. Organizations considering MBSE for enterprise IT applications should anticipate similar challenges. The awareness deficit documented among systems engineering professionals by \citeauthor{Henderson_McDermott_Salado_2024} suggests that awareness among Information Assurance and IT professionals---who operate outside the systems engineering discipline entirely---may prove substantially lower, motivating the investigation this dissertation undertakes.
...
\subsection{MBSE Applications to Information Assurance and DevSecOps}

While MBSE research has focused predominantly upon aerospace and defense system development, recent work explores applications to information assurance and security operations. \citeauthor{CMU_SEI_DevSecOps_2023} argue that DevSecOps pipelines constitute complex socio-technical systems---comprising independently developed and maintained, physically and logically distributed, interoperable components---that require systems engineering treatment~\cite{CMU_SEI_DevSecOps_2023}. The tight integration of business mission, capability delivery, and products using integrated processes, tools, and people increases the attack surface of products under development. As organizations adopt DevSecOps tools and techniques with increased coupling between products and the tools used to build them, the attack surface continues to grow, incorporating segments of the development environment itself.

The research proposes a shift from process-based to property-based security assurance. Traditional cybersecurity assurance relies upon process-based standards such as the NIST Risk Management Framework and SP 800-53 security controls. As systems become more complicated and interconnected, process-based standards alone fail to assure system owners that systems function only as intended under all operational circumstances. \citeauthor{CMU_SEI_DevSecOps_2023} developed a DevSecOps Platform Independent Model using the Unified Architecture Framework and SysML to model pipeline security properties, demonstrating how to construct cybersecurity assurance cases from model-based representations.

The approach decomposes the top-level assurance claim---that the DevSecOps pipeline functions only as intended---into ten sub-claims mapped to specific DevSecOps capabilities: planning and tracking, quality assurance, software assurance, configuration management, solution development, integration, verification and validation, deployment, monitoring and control, and hosting. When MBSE-based assurance is implemented effectively, overall risks associated with the DevSecOps pipeline and associated products are reduced, and compliance and legal requirements are naturally addressed within the engineering lifecycle~\cite{CMU_SEI_DevSecOps_2023}. This finding directly supports the theoretical premise of this dissertation: that Digital Engineering capabilities can integrate security engineering into development and operational processes rather than treating compliance as a separate activity conducted after the fact.

The framework was designed for heavily regulated and cybersecurity-constrained environments including defense, banking, and healthcare---domains where Information Assurance professionals operate and where compliance verification demands consume substantial organizational resources. The research argues that DevSecOps pipelines should become part of enterprise architecture rather than remaining separate from overall system architecture and compliance contexts. This integration addresses precisely the documentation-reality gap and organizational silo challenges documented in Sections~\ref{chap:Chapter_1} and~\ref{sec:enterprise_visibility}. The CMU SEI framework represents a conceptual and architectural argument rather than empirical validation; the research demonstrates technical feasibility through detailed modeling of configuration management capabilities but does not provide quantitative effectiveness data. Nevertheless, the framework establishes that MBSE application to information assurance and security operations has received serious technical investigation from an authoritative institution, distinguishing this domain from the complete research absence that characterizes enterprise IT infrastructure applications.

\subsection{The Absence of MBSE for Enterprise IT}

The literature review reveals a striking gap: no peer-reviewed research addresses MBSE application to enterprise IT infrastructure management, and MBSE application to Information Assurance remains limited to the preliminary DevSecOps security assurance framework discussed in the preceding subsection. With the sole exception of the reference model by \citeauthor{Bonar_Hastings_2024}, the academic literature contains no studies examining whether MBSE approaches could improve IT service management or support compliance verification in enterprise contexts broadly~\cite{Bonar_Hastings_2024}.
...
Empirical research confirms that organizational structure significantly influences MBSE adoption outcomes. \citeauthor{Henderson_Salado_2024_Org} surveyed fifty-one practitioners to examine correlations between seven organizational structure variables and measures of adoption and implementation~\cite{Henderson_Salado_2024_Org}. Flexibility and interconnectedness---the degree to which organizational units communicate and collaborate across boundaries---showed the strongest positive correlations with both adoption processes and implementation maturity. Centralization correlated negatively with adoption, suggesting that highly centralized governance structures may impede Digital Engineering adoption. Counterintuitively, formalization---the degree to which organizations maintain documented procedures and processes---correlated positively with adoption. This finding indicates that existing compliance frameworks such as NIST RMF or ITIL could provide organizational scaffolding that supports Digital Engineering adoption when paired with sufficient flexibility and cross-functional communication. The absence of significant correlation between specialization and adoption suggests that the disciplinary boundary between systems engineering and IT may not constitute as rigid a barrier as organizational structure variables.

Qualitative evidence reinforces the structural findings. \citeauthor{Henderson_McDermott_Salado_2024} found that successful MBSE adopters commonly established Core MBSE teams functioning as communities of practice or centers of excellence, implemented role-based training at four levels---model reviewers, developers and modelers, architects, and administrators---and maintained standardization groups or governance frameworks~\cite{Henderson_McDermott_Salado_2024}. A reinforcing dynamic emerged across the interview data: as stakeholders gained exposure to models, they perceived greater benefits, which in turn generated broader organizational buy-in. This pattern suggests that enterprise IT organizations could leverage familiar organizational structures---Centers of Excellence, role-based training programs, governance committees---to support Digital Engineering adoption rather than requiring wholesale organizational transformation. Two-thirds of interview participants reported that their organizations had established some form of standardization group or framework to guide MBSE implementation, further reinforcing the positive role of formalization documented in the quantitative analysis.
...
Measured evidence of positive return on investment exists but remains rare in the literature. \citeauthor{Rogers_Mitchell_2021} documented the outcomes of a three million dollar MBSE investment over two and a half years on the Submarine Warfare Federated Tactical Systems program, a rapidly evolving combat system of systems comprising over ten million source lines of code deployed across 104 submarines~\cite{Rogers_Mitchell_2021}. An apples-to-apples comparison between legacy document-centric processes and the MBSE approach revealed that systems engineering hours per requirement decreased from 12.1 to 9.9---an eighteen percent improvement in efficiency that exceeded the thirteen percent improvement projected by an earlier pilot study. The MBSE approach handled forty-two percent more interface requirements changes while consuming only sixteen percent more hours, reduced total interface defects by nine percent, and shifted eighteen percent of defects to earlier discovery phases where correction costs between 1.6 and 4 times less than defects discovered during platform integration testing. Supplementary data showed thirty percent more baselines produced monthly and sixty percent more integrated subsystems and combat system variants managed within constant resources.

Despite this evidence, the return on investment for Digital Engineering in enterprise IT contexts remains undemonstrated. \citeauthor{Henderson_McDermott_Salado_2024} note that insufficient empirical evidence demonstrating the value of MBSE persists as a concern among practitioners, with the economic question remaining one of the most frequently cited barriers~\cite{Henderson_McDermott_Salado_2024}. \citeauthor{Call_2024} explain this paradox through Diffusion of Innovations theory: MBSE functions as a preventative innovation whose advantages derive from preventing problems rather than producing visible new benefits~\cite{Call_2024}. The up-front cost spike during adoption delays benefit realization, depressing perceived relative advantage even when objective value exists. Aerospace and defense organizations can cite program outcomes to justify continued investment. Enterprise IT organizations have no comparable evidence because no research examines Digital Engineering benefits in these contexts. For enterprise IT organizations operating on annual budgets with continuous delivery expectations, these perception barriers compound the challenge of justifying investment without domain-specific evidence. Without evidence, investment decisions favor proven approaches over experimental adoptions.
...
\section{Enterprise Visibility and Dependency Documentation Failures}\label{sec:enterprise_visibility}
...
\begin{table}[htbp]
  ...
  \begin{tabular}{@{}p{1.8in}p{2.1in}p{2.2in}@{}}
    ...
    \midrule
    Compliance & Preliminary MBSE-DevSecOps framework exists; no DE approaches for RMF compliance broadly & Practical implementation studies \\
    \midrule
...
The literature review establishes clear justification for this research through multiple converging factors. Digital Engineering has demonstrated value in traditional systems engineering domains, including rare measured evidence of positive return on investment~\cite{Rogers_Mitchell_2021}. Compliance frameworks explicitly require enterprise architecture capabilities that current approaches fail to provide. Research documents pervasive failures in current IT documentation and configuration management practices. Preliminary frameworks for applying MBSE to DevSecOps security assurance have been developed~\cite{CMU_SEI_DevSecOps_2023}. Despite these factors, academic research has not investigated Digital Engineering application to enterprise IT and Information Assurance broadly. Furthermore, empirical evidence demonstrates that even within the systems engineering discipline, significant awareness and perception barriers impede MBSE adoption~\cite{Call_2024, Henderson_McDermott_Salado_2024}, raising the question of whether similar or greater barriers exist among professionals in domains that have never encountered these methodologies.
...
This literature review has examined the current body of knowledge across nine interconnected domains relevant to applying Digital Engineering methodologies to Information Assurance and IT Service Management. The review established that Digital Engineering has demonstrated value in aerospace, defense, and manufacturing contexts, with authoritative guidance from organizations including the Department of Defense, NASA, and INCOSE providing mature frameworks and methodologies. Measured evidence of positive return on investment, though rare, documents significant efficiency and quality improvements following MBSE adoption~\cite{Rogers_Mitchell_2021}. Recent empirical research illuminates the dynamics governing adoption, revealing that perceptions of compatibility, complexity, and trialability mediate adoption decisions~\cite{Call_2024}, that organizational structure variables including flexibility and interconnectedness significantly influence adoption outcomes~\cite{Henderson_Salado_2024_Org}, and that awareness deficits persist even among systems engineering professionals~\cite{Henderson_McDermott_Salado_2024}. Preliminary frameworks for applying MBSE to DevSecOps security assurance demonstrate technical feasibility for Information Assurance contexts~\cite{CMU_SEI_DevSecOps_2023}. Yet systematic examination across major academic databases revealed a pronounced research gap: apart from the preliminary reference model by \citeauthor{Bonar_Hastings_2024}~\cite{Bonar_Hastings_2024}, no peer-reviewed research addresses Digital Engineering application to enterprise IT infrastructure, IT Service Management, or Information Assurance programs broadly.
...
The review examined the Unified Architecture Framework as the consolidating standard for enterprise architecture, explaining how OMG developed UAF to unify DoDAF, MODAF, NAF, and commercial frameworks with adoption by the Department of Defense, NATO, and the UK Ministry of Defence. The review analyzed Model-Based Enterprise limitations that impede enterprise IT adoption, identifying scope restrictions, organizational barriers, and skills gaps that prevent MBE practices from extending beyond manufacturing contexts. The review explored barriers to Digital Engineering adoption outside defense and aerospace, documenting platform-centric adoption patterns, organizational separation between IT and engineering functions, economic factors that discourage investment without demonstrated return, and empirical evidence that organizational structure variables including centralization and flexibility significantly influence adoption outcomes~\cite{Henderson_Salado_2024_Org}.
